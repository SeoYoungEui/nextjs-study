...
// 브레이크 포인트
const BREAKPOINT: { [key: string]: string } = {
  sm: '640px', // 640px 이상
  md: '768px', // 768px 이상
  lg: '1024px', // 1024px 이상
  xl: '1280px', // 1280px 이상
 ...
 * @return CSS 속성과 그 값 (ex. background-color: white;)
 */
export function toPropValue<T>(
  ...
  if (isResponsivePropType(prop)) {
    ...
    for (const responsiveKey in prop) {
      if (responsiveKey === 'base') {
        ...
        // 미디어 쿼리의 스타일
        const breakpoint = BREAKPOINT[responsiveKey]
  ...
  return toThemeValueIfNeeded(propKey, prop, theme)
}
